...
| 07 | [Vector Balancing Lab](tools/07-vector-balancing/index.html) | Vectors | The classic force-table lab, top-down: masses hang over pulleys around a ring, each pulling with a force equal to its weight (F = mg). Add or remove up to four forces, drag any mass (0–500 g) or angle, and the ring drifts off-center — colored green/gold/red by how far from equilibrium you are — toward the direction of whatever's left over after every pull is summed. A "Show equilibrant" toggle draws the exact vector (and hanging mass) that would cancel the net force and re-center the ring. |
