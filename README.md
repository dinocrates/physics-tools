...
| 03 | [Vector Products Lab](tools/03-vector-products/index.html) | Vectors | What dot and cross products actually mean, geometrically, in a fully rotatable 3D scene (drag to orbit, scroll to zoom, no libraries — hand-rolled projection on canvas). Dot mode draws the literal "shadow" one vector casts on the other, color-coded by sign. Cross mode draws the parallelogram A and B span and the perpendicular result vector, so you can rotate the scene and confirm it really is perpendicular to both. |
